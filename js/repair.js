(function () {
  ...
  function record() { update(); if (steps.some((name) => !state.draft['step_' + name])) { $('repair-message').textContent = 'Finish the three checks before recording this repair.'; return; } const item = { title: state.draft.title || 'Untitled repair', owner: state.draft.owner || 'AJH', signal: state.draft.signal || '', repair: state.draft.repair || 'Small patch recorded', expected: state.draft.expected || '', notes: state.draft.notes || '', date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) }; state.history.unshift(item); state.history = state.history.slice(0, 12); state.repairs += 1; state.draft = {}; setDraft(); save(); renderHistory(); update(); $('repair-repairs').textContent = state.repairs; $('repair-status').textContent = 'Repair recorded'; $('repair-message').textContent = 'Good. The next build can inspect the recorded patch instead of reopening the whole problem.'; }
