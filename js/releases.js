(function () {
  'use strict';
  const BUILDS = [
    { d: 92, a: 'systems', n: 'Next-Build Brief', i: 4, date: 'Jul 26, 2026', text: 'A local, editable brief turns the Pulse recommendation into one concrete next slice.' },
  ...
  function render() {
    ...
    $('releases-count').textContent = shown.length; $('releases-impact').textContent = `${impact} impact points`; $('releases-scope').textContent = state.fullHistory ? 'Full 92-day archive' : 'Latest 16'; $('releases-history-toggle').innerHTML = `<i class="fas fa-arrows-to-circle"></i> ${state.fullHistory ? 'Show latest 16' : 'Show full history'}`; $('releases-history-toggle').setAttribute('aria-pressed', String(state.fullHistory)); $('releases-history-hint').textContent = state.fullHistory ? 'the latest 16 are the quick view' : 'show the full 92-day archive when you want the long view';
