(function () {
  ...
  function render() {
    ...
    const momentum = recentImpact / (recent.length * 5); const consistency = all.length >= 2 ? Math.min(1, all.length / 95) : .2; const signal = Math.round((momentum * .65 + consistency * .35) * 100);
