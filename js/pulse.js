(function () {
  ...
  function render() {
    ...
    const momentum = recentImpact / (recent.length * 5); const consistency = all.length >= 2 ? Math.min(1, all.length / 91) : .2; const signal = Math.round((momentum * .65 + consistency * .35) * 100);
  ...
  function open() { $('pulse')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); state.views += 1; save(); render(); }
  function refresh() { render(); }
  function boot() { load(); render(); $('pulse-refresh')?.addEventListener('click', refresh); $('pulse-hero-btn')?.addEventListener('click', open); document.addEventListener('keydown', (event) => { if (event.target.matches('input, textarea, select, button, a')) return; if (event.key.toLowerCase() === 'p' && event.shiftKey) open(); }); window.ajhPulse = { open, refresh, state: () => state }; }
