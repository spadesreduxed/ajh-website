(function () {
  ...
  const state = { filter: 'all', selected: 93, theme: 'dark', views: 0 };
  ...
  function updateFocus() {
    ...
    $('cp-focus-name').textContent = b.n; $('cp-focus-arch').textContent = meta.label; $('cp-focus-date').textContent = b.d === 93 ? 'Jul 27, 2026' : b.d === 92 ? 'Jul 26, 2026' : new Date(2026, 3, 21 + b.d + 4).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    $('cp-focus-impact').textContent = 'Impact ' + b.i + '/5'; $('cp-focus-desc').textContent = b.n + ' sits on the ' + meta.label.toLowerCase() + ' axis of the build compass. The map turns the 93-day streak into direction: where the work points, and what to build next.';
  }
  function render() {
