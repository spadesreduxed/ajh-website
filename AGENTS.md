...
**Actions**:
- Extended the Build Compass data model to 85 builds and rolled the hero, streak counters, focus state, and command palette “Today” action forward.
- Added a **Next direction** recommendation card that scores Systems, Shipping, Craft, and Learning from the actual archetype data and points to the least-used direction.
- Added an **Export SVG** button with embedded snapshot styles, plus persisted theme restoration and a working journal jump action.
- Researched browser Blob downloads, SVG accessible naming, and reduced-motion guidance before shipping the export and recommendation UI.

**Files Changed**:
- `index.html` - Day 85 build card, Compass controls/recommendation markup, counters, versioned assets.
- `css/compass.css` - recommendation card and reduced-motion styling.
- `js/compass.js` - Day 85 data, direction scoring, recommendation API, SVG export, theme restoration, and updated bounds.
- `js/main.js` - Day 85 Compass command/bookmark references.
- `README.md` - latest build documentation.

**Validation**:
- `node --check js/compass.js` passes.
- `git diff --check` passes.
- Local preview smoke test completed before commit.

**Next Steps**:
- Keep the daily build moving; the next lens should make the recommendation actionable.
